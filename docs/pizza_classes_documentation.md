...
Generated on: **2025-01-18 11:38:42**
...
<div style="flex: 1 1 calc(33.33% - 20px); min-width: 200px;">
<a href="#pizza_private_test" style="text-decoration: none; font-weight: bold;">
17. pizza.private.test
</a>
</div>
<div style="flex: 1 1 calc(33.33% - 20px); min-width: 200px;">
...
| Class | Method | Docstring First Paragraph | # Lines | __version__ |
|-------|---------|---------------------------|---------|-------------|
| (module-level) | `autoname` | generate automatically names | 3 | 1.002 |
| (module-level) | `get_metadata` | Return a dictionary of explicitly defined metadata. | 15 | 1.002 |
| `ScriptTemplate` | `__getattr__` | Handles attribute retrieval, checking the following in order: 1. If 'name' is in default_attributes, return the value from attributes if it exists, otherwise return the default value from default_attributes. 2. If 'name' is 'content', return the content (or an empty string if content is not set). 3. If 'name' exists in the attributes dictionary, return its value. 4. If attributes itself exists in __dict__, return the value from attributes if 'name' is found. 5. If all previous checks fail, raise an AttributeError indicating that 'name' is not found. | 39 | 1.002 |
| `ScriptTemplate` | `__init__` | Initializes a new `ScriptTemplate` object. | 96 | 1.002 |
| `ScriptTemplate` | `__repr__` | Return repr(self). | 87 | 1.002 |
| `ScriptTemplate` | `__setattr__` | Implement setattr(self, name, value). | 24 | 1.002 |
| `ScriptTemplate` | `__str__` | Return str(self). | 3 | 1.002 |
| `ScriptTemplate` | `_calculate_content_hash` | Generate hash for content. | 3 | 1.002 |
| `ScriptTemplate` | `_invalidate_cache` | Reset all cache entries. | 10 | 1.002 |
| `ScriptTemplate` | `_update_content` | Helper to set _content and _content_hash, refreshing cache as necessary. | 19 | 1.002 |
| `ScriptTemplate` | `check_variables` | Checks for undefined variables in the ScriptTemplate instance. | 56 | 1.002 |
| `ScriptTemplate` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 46 | 1.002 |
| `ScriptTemplate` | `do` | Executes or prepares the script template content based on its attributes and the `softrun` flag. | 103 | 1.002 |
| `ScriptTemplate` | `is_variable_defined` | Checks if a specified variable is defined (either as a default value or a set value). | 29 | 1.002 |
| `ScriptTemplate` | `is_variable_set_value_only` | Checks if a specified variable is defined and set to a specific (non-default) value. | 31 | 1.002 |
| `ScriptTemplate` | `refreshvar` | Detects variables in the content and adds them to definitions if needed. This method ensures that variables like ${varname} are correctly detected and added to the definitions if they are missing. | 14 | 1.002 |
| `dscript` | `__add__` | Concatenates two dscript objects, creating a new dscript object that combines the TEMPLATE and DEFINITIONS of both. This operation avoids deep copying of definitions by creating a new lambdaScriptdata instance from the definitions. | 55 | 1.002 |
| `dscript` | `__call__` | Extracts subobjects from the dscript based on the provided keys. | 35 | 1.002 |
| `dscript` | `__contains__` |  | 2 | 1.002 |
| `dscript` | `__copy__` | copy method | 6 | 1.002 |
| `dscript` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `dscript` | `__delitem__` |  | 2 | 1.002 |
| `dscript` | `__getattr__` |  | 16 | 1.002 |
| `dscript` | `__getitem__` | Implements index-based retrieval, slicing, or reordering for dscript objects. | 48 | 1.002 |
| `dscript` | `__init__` | Initializes a new `dscript` object. | 65 | 1.002 |
| `dscript` | `__iter__` |  | 2 | 1.002 |
| `dscript` | `__len__` |  | 2 | 1.002 |
| `dscript` | `__or__` | Pipes a dscript object with other objects. When other is a dscript object, both objects are concatenated (+) before being converted into a pipescript object When other is a script, pipescript or scriptobjectgroup, self is converted into a pipescript | 29 | 1.002 |
| `dscript` | `__repr__` | Representation of dscript object with additional properties. | 48 | 1.002 |
| `dscript` | `__setattr__` | Implement setattr(self, name, value). | 24 | 1.002 |
| `dscript` | `__setitem__` |  | 7 | 1.002 |
| `dscript` | `__str__` | Return str(self). | 4 | 1.002 |
| `dscript` | `_build_html_table` | Helper method to build an HTML table with embedded CSS. | 98 | 1.002 |
| `dscript` | `_escape_html` | Helper method to escape HTML special characters in text. | 16 | 1.002 |
| `dscript` | `_format_field` | Helper method to format individual fields for the table. | 20 | 1.002 |
| `dscript` | `_format_list` | Helper method to format list-type fields for the table. | 17 | 1.002 |
| `dscript` | `_format_values` | Helper method to format the 'values' field, which is a list of tuples. | 17 | 1.002 |
| `dscript` | `add_dynamic_script` | Add a dynamic script step to the dscript object. | 33 | 1.002 |
| `dscript` | `check_all_variables` | Checks for undefined variables for each TEMPLATE key in the dscript object. | 38 | 1.002 |
| `dscript` | `clean` | Clean the TEMPLATE by removing or fixing empty steps. | 42 | 1.002 |
| `dscript` | `createEmptyVariables` | Creates empty variables in DEFINITIONS if they don't already exist. | 14 | 1.002 |
| `dscript` | `detect_all_variables` | Detects all variables across all templates in the dscript object. | 20 | 1.002 |
| `dscript` | `do` | Executes or previews all `ScriptTemplate` instances in `TEMPLATE`, concatenating their processed content. Allows for optional headers and footers based on verbosity settings, and offers a preliminary preview mode with `softrun`. Accumulates definitions across all templates if `return_definitions=True`. | 101 | 1.002 |
| `dscript` | `flattenvariables` | Flatten the variable definitions for each step based on usage and precedence. | 60 | 1.002 |
| `dscript` | `generator` | Returns ------- STR generated code corresponding to dscript (using dscript syntax/language). | 9 | 1.002 |
| `dscript` | `get_attributes_by_index` | Returns the attributes of the ScriptTemplate at the specified index. | 4 | 1.002 |
| `dscript` | `get_content_by_index` | Returns the content of the ScriptTemplate at the specified index. | 45 | 1.002 |
| `dscript` | `header` | Generate a formatted header for the DSCRIPT file. | 63 | 1.002 |
| `dscript` | `items` |  | 2 | 1.002 |
| `dscript` | `keys` | Return the keys of the TEMPLATE. | 3 | 1.002 |
| `dscript` | `list_values` | List all unique values taken by a specified key across global definitions and all steps in sequential order. | 179 | 1.002 |
| `dscript` | `pipescript` | Returns a pipescript object by combining script objects corresponding to the given keys. | 71 | 1.002 |
| `dscript` | `print_var_info` | Print or save a neatly formatted table of variable information based on the analysis from `var_info()`. | 205 | 1.002 |
| `dscript` | `reorder` | Reorder the TEMPLATE lines according to a list of indices. | 10 | 1.002 |
| `dscript` | `save` | Save the current script instance to a text file. | 222 | 1.002 |
| `dscript` | `script` | returns the corresponding script | 10 | 1.002 |
| `dscript` | `search` | Search for foreign/definition key values associated with given primary key/definition value(s). | 193 | 1.002 |
| `dscript` | `set_all_variables` | Ensures that all variables in the templates are added to the global definitions with default values if they are not already defined. | 11 | 1.002 |
| `dscript` | `values` | Return the ScriptTemplate objects in TEMPLATE. | 3 | 1.002 |
| `dscript` | `var_info` | Analyze and gather comprehensive information about variables used in the script. | 130 | 1.002 |
| `dscript` | `write` | Writes the provided script content to a specified file in a given folder, with a header if necessary. | 86 | 1.002 |
| `lambdaScriptdata` | `__init__` | Constructor for lambdaScriptdata. It forces the parent's _returnerror parameter to False. | 19 | 1.002 |
| `lamdaScript` | `__init__` | Initialize a new `lambdaScript` instance. | 66 | 1.002 |
...
<a href="#table_of_contents" title="Back to Table of Contents" style="text-decoration: none;">⬆️ TOC</a>
<a href="#pizza_private_test" title="Go to Next Module: pizza.private.test" style="text-decoration: none;">➡️ Next</a>
</div>
...
**[Class Examples for `pizza/private/mstruct.py` (11)](class_examples.html#pizza_private_mstruct)**

### Methods Table

| Class | Method | Docstring First Paragraph | # Lines | __version__ |
|-------|---------|---------------------------|---------|-------------|
| `param` | `__add__` | add a structure set sortdefintions=True to sort definitions (to maintain executability) | 10 | 1.002 |
| `param` | `__call__` | Extract a sub-structure based on the specified keys, keeping the same class type. | 35 | 1.002 |
| `param` | `__contains__` | in override | 3 | 1.002 |
| `param` | `__copy__` | copy method | 6 | 1.002 |
| `param` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `param` | `__delattr__` | Delete an instance attribute if it exists and is not a class or excluded attribute. | 10 | 1.002 |
| `param` | `__getattr__` | get attribute override | 3 | 1.002 |
| `param` | `__getitem__` | s[i] returns the ith element of the structure s[:4] returns a structure with the four first fields s[[1,3]] returns the second and fourth elements | 26 | 1.002 |
| `param` | `__getstate__` | getstate for cooperative inheritance / duplication | 3 | 1.002 |
| `param` | `__iadd__` | iadd a structure set sortdefintions=True to sort definitions (to maintain executability) | 9 | 1.002 |
| `param` | `__init__` | constructor | 7 | 1.002 |
| `param` | `__isub__` | isub a structure | 9 | 1.002 |
| `param` | `__iter__` | struct iterator | 6 | 1.002 |
| `param` | `__len__` | return the number of fields | 4 | 1.002 |
| `param` | `__next__` | increment iterator | 7 | 1.002 |
| `param` | `__repr__` | display method | 52 | 1.002 |
| `param` | `__setattr__` | set attribute override | 3 | 1.002 |
| `param` | `__setitem__` | set the ith element of the structure | 24 | 1.002 |
| `param` | `__setstate__` | setstate for cooperative inheritance / duplication | 3 | 1.002 |
| `param` | `__str__` | Return str(self). | 2 | 1.002 |
| `param` | `__sub__` | sub a structure | 10 | 1.002 |
| `param` | `check` | populate fields from a default structure check(defaultstruct) missing field, None and [] values are replaced by default ones | 19 | 1.002 |
| `param` | `clear` | clear() delete all fields while preserving the original class | 3 | 1.002 |
| `param` | `dict2struct` | create a structure from a dictionary | 8 | 1.002 |
| `param` | `disp` | display method | 3 | 1.002 |
| `param` | `dispmax` | optimize display | 8 | 1.002 |
| `param` | `escape` | escape \${} as ${{}} --> keep variable names convert ${} as {} --> prepare Python replacement | 35 | 1.002 |
| `param` | `eval` | Eval method for structure such as MS.alias | 92 | 1.002 |
| `param` | `format` | format a string with field (use {field} as placeholders) s.replace(string), s.replace(string,escape=True) where: s is a struct object string is a string with possibly ${variable1} escape is a flag to prevent ${} replaced by {} | 27 | 1.002 |
| `param` | `formateval` | format method with evaluation feature | 44 | 1.002 |
| `param` | `fromkeys` | returns a structure from keys | 3 | 1.002 |
| `param` | `fromkeysvalues` | struct.keysvalues(keys,values) creates a structure from keys and values use makeparam = True to create a param instead of struct | 18 | 1.002 |
| `param` | `generator` | generate Python code of the equivalent structure | 21 | 1.002 |
| `param` | `getattr` | Get attribute override to access both instance attributes and properties if allowed. | 11 | 1.002 |
| `param` | `hasattr` | Return true if the field exists, considering properties as regular attributes if allowed. | 7 | 1.002 |
| `param` | `isdefined` | isdefined(ref) returns true if it is defined in ref | 19 | 1.002 |
| `param` | `isstrdefined` | isstrdefined(string,ref) returns true if it is defined in ref | 14 | 1.002 |
| `param` | `isstrexpression` | isstrexpression(string) returns true if s contains an expression | 5 | 1.002 |
| `param` | `items` | return all elements as iterable key, value | 3 | 1.002 |
| `param` | `keys` | return the fields | 4 | 1.002 |
| `param` | `keyssorted` | sort keys by length() | 5 | 1.002 |
| `param` | `protect` | protect $variable as ${variable} | 11 | 1.002 |
| `param` | `read` | read the equivalent structure read(filename) | 35 | 1.002 |
| `param` | `scan` | scan(string) scan a string for variables | 11 | 1.002 |
| `param` | `set` | initialization | 3 | 1.002 |
| `param` | `setattr` | set field and value | 6 | 1.002 |
| `param` | `sortdefinitions` | sortdefintions sorts all definitions so that they can be executed as param(). If any inconsistency is found, an error message is generated. | 53 | 1.002 |
| `param` | `struct2dict` | create a dictionary from the current structure | 3 | 1.002 |
| `param` | `struct2param` | convert an object struct() to param() | 8 | 1.002 |
| `param` | `tostatic` | convert dynamic a param() object to a static struct() object. note: no interpretation note: use tostruct() to interpret them and convert it to struct note: tostatic().struct2param() makes it reversible | 7 | 1.002 |
| `param` | `tostruct` | generate the evaluated structure tostruct(protection=False) | 6 | 1.002 |
| `param` | `update` | Update multiple fields at once, while protecting certain attributes. | 22 | 1.002 |
| `param` | `values` | return the values | 4 | 1.002 |
| `param` | `write` | write the equivalent structure (not recursive for nested struct) write(filename, overwrite=True, mkdir=False) | 33 | 1.002 |
| `param` | `zip` | zip keys and values | 3 | 1.002 |
| `paramauto` | `__add__` | add a structure set sortdefintions=True to sort definitions (to maintain executability) | 2 | 1.002 |
| `paramauto` | `__call__` | Extract a sub-structure based on the specified keys, keeping the same class type. | 35 | 1.002 |
| `paramauto` | `__contains__` | in override | 3 | 1.002 |
| `paramauto` | `__copy__` | copy method | 6 | 1.002 |
| `paramauto` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `paramauto` | `__delattr__` | Delete an instance attribute if it exists and is not a class or excluded attribute. | 10 | 1.002 |
| `paramauto` | `__getattr__` | get attribute override | 3 | 1.002 |
| `paramauto` | `__getitem__` | s[i] returns the ith element of the structure s[:4] returns a structure with the four first fields s[[1,3]] returns the second and fourth elements | 26 | 1.002 |
| `paramauto` | `__getstate__` | getstate for cooperative inheritance / duplication | 3 | 1.002 |
| `paramauto` | `__iadd__` | iadd a structure set sortdefintions=True to sort definitions (to maintain executability) | 2 | 1.002 |
| `paramauto` | `__init__` | constructor | 7 | 1.002 |
| `paramauto` | `__isub__` | isub a structure | 9 | 1.002 |
| `paramauto` | `__iter__` | struct iterator | 6 | 1.002 |
| `paramauto` | `__len__` | return the number of fields | 4 | 1.002 |
| `paramauto` | `__next__` | increment iterator | 7 | 1.002 |
| `paramauto` | `__repr__` | display method | 5 | 1.002 |
| `paramauto` | `__setattr__` | set attribute override | 3 | 1.002 |
| `paramauto` | `__setitem__` | set the ith element of the structure | 24 | 1.002 |
| `paramauto` | `__setstate__` | setstate for cooperative inheritance / duplication | 3 | 1.002 |
| `paramauto` | `__str__` | Return str(self). | 2 | 1.002 |
| `paramauto` | `__sub__` | sub a structure | 10 | 1.002 |
| `paramauto` | `check` | populate fields from a default structure check(defaultstruct) missing field, None and [] values are replaced by default ones | 19 | 1.002 |
| `paramauto` | `clear` | clear() delete all fields while preserving the original class | 3 | 1.002 |
| `paramauto` | `dict2struct` | create a structure from a dictionary | 8 | 1.002 |
| `paramauto` | `disp` | display method | 3 | 1.002 |
| `paramauto` | `dispmax` | optimize display | 8 | 1.002 |
| `paramauto` | `escape` | escape \${} as ${{}} --> keep variable names convert ${} as {} --> prepare Python replacement | 35 | 1.002 |
| `paramauto` | `eval` | Eval method for structure such as MS.alias | 92 | 1.002 |
| `paramauto` | `format` | format a string with field (use {field} as placeholders) s.replace(string), s.replace(string,escape=True) where: s is a struct object string is a string with possibly ${variable1} escape is a flag to prevent ${} replaced by {} | 27 | 1.002 |
| `paramauto` | `formateval` | format method with evaluation feature | 44 | 1.002 |
| `paramauto` | `fromkeys` | returns a structure from keys | 3 | 1.002 |
| `paramauto` | `fromkeysvalues` | struct.keysvalues(keys,values) creates a structure from keys and values use makeparam = True to create a param instead of struct | 18 | 1.002 |
| `paramauto` | `generator` | generate Python code of the equivalent structure | 21 | 1.002 |
| `paramauto` | `getattr` | Get attribute override to access both instance attributes and properties if allowed. | 11 | 1.002 |
| `paramauto` | `hasattr` | Return true if the field exists, considering properties as regular attributes if allowed. | 7 | 1.002 |
| `paramauto` | `isdefined` | isdefined(ref) returns true if it is defined in ref | 19 | 1.002 |
| `paramauto` | `isstrdefined` | isstrdefined(string,ref) returns true if it is defined in ref | 14 | 1.002 |
| `paramauto` | `isstrexpression` | isstrexpression(string) returns true if s contains an expression | 5 | 1.002 |
| `paramauto` | `items` | return all elements as iterable key, value | 3 | 1.002 |
| `paramauto` | `keys` | return the fields | 4 | 1.002 |
| `paramauto` | `keyssorted` | sort keys by length() | 5 | 1.002 |
| `paramauto` | `protect` | protect $variable as ${variable} | 11 | 1.002 |
| `paramauto` | `read` | read the equivalent structure read(filename) | 35 | 1.002 |
| `paramauto` | `scan` | scan(string) scan a string for variables | 11 | 1.002 |
| `paramauto` | `set` | initialization | 3 | 1.002 |
| `paramauto` | `setattr` | set field and value | 6 | 1.002 |
| `paramauto` | `sortdefinitions` | sortdefintions sorts all definitions so that they can be executed as param(). If any inconsistency is found, an error message is generated. | 53 | 1.002 |
| `paramauto` | `struct2dict` | create a dictionary from the current structure | 3 | 1.002 |
| `paramauto` | `struct2param` | convert an object struct() to param() | 8 | 1.002 |
| `paramauto` | `tostatic` | convert dynamic a param() object to a static struct() object. note: no interpretation note: use tostruct() to interpret them and convert it to struct note: tostatic().struct2param() makes it reversible | 7 | 1.002 |
| `paramauto` | `tostruct` | generate the evaluated structure tostruct(protection=False) | 6 | 1.002 |
| `paramauto` | `update` | Update multiple fields at once, while protecting certain attributes. | 22 | 1.002 |
| `paramauto` | `values` | return the values | 4 | 1.002 |
| `paramauto` | `write` | write the equivalent structure (not recursive for nested struct) write(filename, overwrite=True, mkdir=False) | 33 | 1.002 |
| `paramauto` | `zip` | zip keys and values | 3 | 1.002 |
| `pstr` | `__add__` | Return self+value. | 2 | 1.002 |
| `pstr` | `__iadd__` |  | 2 | 1.002 |
| `pstr` | `__repr__` | Return repr(self). | 5 | 1.002 |
| `pstr` | `__truediv__` | overload / | 7 | 1.002 |
| `pstr` | `eval` | evaluate the path of it os a path | 9 | 1.002 |
| `pstr` | `topath` | return a validated path | 6 | 1.002 |
| `struct` | `__add__` | add a structure set sortdefintions=True to sort definitions (to maintain executability) | 10 | 1.002 |
| `struct` | `__call__` | Extract a sub-structure based on the specified keys, keeping the same class type. | 35 | 1.002 |
| `struct` | `__contains__` | in override | 3 | 1.002 |
| `struct` | `__copy__` | copy method | 6 | 1.002 |
| `struct` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `struct` | `__delattr__` | Delete an instance attribute if it exists and is not a class or excluded attribute. | 10 | 1.002 |
| `struct` | `__getattr__` | get attribute override | 3 | 1.002 |
| `struct` | `__getitem__` | s[i] returns the ith element of the structure s[:4] returns a structure with the four first fields s[[1,3]] returns the second and fourth elements | 26 | 1.002 |
| `struct` | `__getstate__` | getstate for cooperative inheritance / duplication | 3 | 1.002 |
| `struct` | `__iadd__` | iadd a structure set sortdefintions=True to sort definitions (to maintain executability) | 9 | 1.002 |
| `struct` | `__init__` | constructor | 5 | 1.002 |
| `struct` | `__isub__` | isub a structure | 9 | 1.002 |
| `struct` | `__iter__` | struct iterator | 6 | 1.002 |
| `struct` | `__len__` | return the number of fields | 4 | 1.002 |
| `struct` | `__next__` | increment iterator | 7 | 1.002 |
| `struct` | `__repr__` | display method | 52 | 1.002 |
| `struct` | `__setattr__` | set attribute override | 3 | 1.002 |
| `struct` | `__setitem__` | set the ith element of the structure | 24 | 1.002 |
| `struct` | `__setstate__` | setstate for cooperative inheritance / duplication | 3 | 1.002 |
| `struct` | `__str__` | Return str(self). | 2 | 1.002 |
| `struct` | `__sub__` | sub a structure | 10 | 1.002 |
| `struct` | `check` | populate fields from a default structure check(defaultstruct) missing field, None and [] values are replaced by default ones | 19 | 1.002 |
| `struct` | `clear` | clear() delete all fields while preserving the original class | 3 | 1.002 |
| `struct` | `dict2struct` | create a structure from a dictionary | 8 | 1.002 |
| `struct` | `disp` | display method | 3 | 1.002 |
| `struct` | `dispmax` | optimize display | 8 | 1.002 |
| `struct` | `format` | format a string with field (use {field} as placeholders) s.replace(string), s.replace(string,escape=True) where: s is a struct object string is a string with possibly ${variable1} escape is a flag to prevent ${} replaced by {} | 27 | 1.002 |
| `struct` | `fromkeys` | returns a structure from keys | 3 | 1.002 |
| `struct` | `fromkeysvalues` | struct.keysvalues(keys,values) creates a structure from keys and values use makeparam = True to create a param instead of struct | 18 | 1.002 |
| `struct` | `generator` | generate Python code of the equivalent structure | 21 | 1.002 |
| `struct` | `getattr` | Get attribute override to access both instance attributes and properties if allowed. | 11 | 1.002 |
| `struct` | `hasattr` | Return true if the field exists, considering properties as regular attributes if allowed. | 7 | 1.002 |
| `struct` | `isdefined` | isdefined(ref) returns true if it is defined in ref | 19 | 1.002 |
| `struct` | `isstrdefined` | isstrdefined(string,ref) returns true if it is defined in ref | 14 | 1.002 |
| `struct` | `isstrexpression` | isstrexpression(string) returns true if s contains an expression | 5 | 1.002 |
| `struct` | `items` | return all elements as iterable key, value | 3 | 1.002 |
| `struct` | `keys` | return the fields | 4 | 1.002 |
| `struct` | `keyssorted` | sort keys by length() | 5 | 1.002 |
| `struct` | `read` | read the equivalent structure read(filename) | 35 | 1.002 |
| `struct` | `scan` | scan(string) scan a string for variables | 11 | 1.002 |
| `struct` | `set` | initialization | 3 | 1.002 |
| `struct` | `setattr` | set field and value | 6 | 1.002 |
| `struct` | `sortdefinitions` | sortdefintions sorts all definitions so that they can be executed as param(). If any inconsistency is found, an error message is generated. | 53 | 1.002 |
| `struct` | `struct2dict` | create a dictionary from the current structure | 3 | 1.002 |
| `struct` | `struct2param` | convert an object struct() to param() | 8 | 1.002 |
| `struct` | `update` | Update multiple fields at once, while protecting certain attributes. | 22 | 1.002 |
| `struct` | `values` | return the values | 4 | 1.002 |
| `struct` | `write` | write the equivalent structure (not recursive for nested struct) write(filename, overwrite=True, mkdir=False) | 33 | 1.002 |
| `struct` | `zip` | zip keys and values | 3 | 1.002 |

<a id="pizza_private_test" name="pizza_private_test"></a>
...
| Class | Method | Docstring First Paragraph | # Lines | __version__ |
|-------|---------|---------------------------|---------|-------------|
| (module-level) | `safe_convert_to_list` |  | 7 |  |

<a id="pizza_private_utils" name="pizza_private_utils"></a>
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; font-size: 0.8em;"><a href="#pizza_private_test" title="Go to Previous Module: pizza.private.test" style="text-decoration: none;">⬅️ Previous</a>
<a href="#table_of_contents" title="Back to Table of Contents" style="text-decoration: none;">⬆️ TOC</a>
...
| Class | Method | Docstring First Paragraph | # Lines | __version__ |
|-------|---------|---------------------------|---------|-------------|
| (module-level) | `frame_header` | Format the header content into an ASCII framed box with customizable properties. | 147 | 1.002 |
| (module-level) | `get_metadata` | Return a dictionary of explicitly defined metadata. | 15 | 1.002 |
| (module-level) | `<lambda>` |  | 1 | 1.002 |
| (module-level) | `is_scalar` | Determines if a value is scalar (not a list, dict, or tuple). | 5 | 1.002 |
| (module-level) | `make_hashable` | Recursively converts lists and dictionaries to tuples to make them hashable. | 9 | 1.002 |
| (module-level) | `picker` |  | 1 | 1.002 |
| (module-level) | `remove_comments` | Removes comments from a single or multi-line string, handling quotes, escaped characters, and line continuation. | 101 | 1.002 |
| (module-level) | `span` |  | 2 | 1.002 |
| `CallableScript` | `__call__` | Call self as a function. | 3 | 1.002 |
| `CallableScript` | `__get__` |  | 3 | 1.002 |
| `CallableScript` | `__init__` | Initialize self.  See help(type(self)) for accurate signature. | 2 | 1.002 |
| `VariableOccurrences` | `__init__` | Initializes the VariableOccurrences object. | 38 | 1.002 |
| `VariableOccurrences` | `_determine_scopes` | Determines the unique scopes present across all variables. | 7 | 1.002 |
| `VariableOccurrences` | `export` | Exports the variable occurrences to a file or returns the content as a string. | 161 | 1.002 |
| `VariableOccurrences` | `get_all_elements_in_lists` | Retrieves all unique elements within list-type variable values. | 26 | 1.002 |
| `VariableOccurrences` | `get_all_values` | Retrieves all unique values of the variable(s). | 26 | 1.002 |
| `VariableOccurrences` | `get_raw_data` | Returns the raw data. | 8 | 1.002 |
| `VariableOccurrences` | `get_steps_with_value` | Retrieves the steps where the variable equals the specified value. | 52 | 1.002 |
| `VariableOccurrences` | `get_steps_with_value_in_scope` | Retrieves the steps within a specific scope where the variable equals the specified value. | 45 | 1.002 |
| `VariableOccurrences` | `get_usage_count` | Counts how many times a specific value is used. | 48 | 1.002 |
| `VariableOccurrences` | `summarize` | Provides a summary of the variable occurrences. | 72 | 1.002 |
| `boundarysection` | `__add__` | overload addition operator | 24 | 1.002 |
| `boundarysection` | `__and__` | overload and operator | 7 | 1.002 |
| `boundarysection` | `__copy__` | copy method | 6 | 1.002 |
| `boundarysection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `boundarysection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `boundarysection` | `__mul__` | overload * operator | 8 | 1.002 |
| `boundarysection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `boundarysection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `boundarysection` | `__repr__` | disp method | 22 | 1.002 |
| `boundarysection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `boundarysection` | `__str__` | string representation | 3 | 1.002 |
| `boundarysection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `boundarysection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `boundarysection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `boundarysection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `boundarysection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `boundarysection` | `printheader` | print header | 7 | 1.002 |
| `boundarysection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `boundarysection` | `write` | Write the script to a file. | 39 | 1.002 |
| `discretizationsection` | `__add__` | overload addition operator | 24 | 1.002 |
| `discretizationsection` | `__and__` | overload and operator | 7 | 1.002 |
| `discretizationsection` | `__copy__` | copy method | 6 | 1.002 |
| `discretizationsection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `discretizationsection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `discretizationsection` | `__mul__` | overload * operator | 8 | 1.002 |
| `discretizationsection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `discretizationsection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `discretizationsection` | `__repr__` | disp method | 22 | 1.002 |
| `discretizationsection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `discretizationsection` | `__str__` | string representation | 3 | 1.002 |
| `discretizationsection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `discretizationsection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `discretizationsection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `discretizationsection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `discretizationsection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `discretizationsection` | `printheader` | print header | 7 | 1.002 |
| `discretizationsection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `discretizationsection` | `write` | Write the script to a file. | 39 | 1.002 |
| `dumpsection` | `__add__` | overload addition operator | 24 | 1.002 |
| `dumpsection` | `__and__` | overload and operator | 7 | 1.002 |
| `dumpsection` | `__copy__` | copy method | 6 | 1.002 |
| `dumpsection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `dumpsection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `dumpsection` | `__mul__` | overload * operator | 8 | 1.002 |
| `dumpsection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `dumpsection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `dumpsection` | `__repr__` | disp method | 22 | 1.002 |
| `dumpsection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `dumpsection` | `__str__` | string representation | 3 | 1.002 |
| `dumpsection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `dumpsection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `dumpsection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `dumpsection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `dumpsection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `dumpsection` | `printheader` | print header | 7 | 1.002 |
| `dumpsection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `dumpsection` | `write` | Write the script to a file. | 39 | 1.002 |
| `geometrysection` | `__add__` | overload addition operator | 24 | 1.002 |
| `geometrysection` | `__and__` | overload and operator | 7 | 1.002 |
| `geometrysection` | `__copy__` | copy method | 6 | 1.002 |
| `geometrysection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `geometrysection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `geometrysection` | `__mul__` | overload * operator | 8 | 1.002 |
| `geometrysection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `geometrysection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `geometrysection` | `__repr__` | disp method | 22 | 1.002 |
| `geometrysection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `geometrysection` | `__str__` | string representation | 3 | 1.002 |
| `geometrysection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `geometrysection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `geometrysection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `geometrysection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `geometrysection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `geometrysection` | `printheader` | print header | 7 | 1.002 |
| `geometrysection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `geometrysection` | `write` | Write the script to a file. | 39 | 1.002 |
| `globalsection` | `__add__` | overload addition operator | 24 | 1.002 |
| `globalsection` | `__and__` | overload and operator | 7 | 1.002 |
| `globalsection` | `__copy__` | copy method | 6 | 1.002 |
| `globalsection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `globalsection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `globalsection` | `__mul__` | overload * operator | 8 | 1.002 |
| `globalsection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `globalsection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `globalsection` | `__repr__` | disp method | 22 | 1.002 |
| `globalsection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `globalsection` | `__str__` | string representation | 3 | 1.002 |
| `globalsection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `globalsection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `globalsection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `globalsection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `globalsection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `globalsection` | `printheader` | print header | 7 | 1.002 |
| `globalsection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `globalsection` | `write` | Write the script to a file. | 39 | 1.002 |
| `initializesection` | `__add__` | overload addition operator | 24 | 1.002 |
| `initializesection` | `__and__` | overload and operator | 7 | 1.002 |
| `initializesection` | `__copy__` | copy method | 6 | 1.002 |
| `initializesection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `initializesection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `initializesection` | `__mul__` | overload * operator | 8 | 1.002 |
| `initializesection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `initializesection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `initializesection` | `__repr__` | disp method | 22 | 1.002 |
| `initializesection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `initializesection` | `__str__` | string representation | 3 | 1.002 |
| `initializesection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `initializesection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `initializesection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `initializesection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `initializesection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `initializesection` | `printheader` | print header | 7 | 1.002 |
| `initializesection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `initializesection` | `write` | Write the script to a file. | 39 | 1.002 |
| `integrationsection` | `__add__` | overload addition operator | 24 | 1.002 |
| `integrationsection` | `__and__` | overload and operator | 7 | 1.002 |
| `integrationsection` | `__copy__` | copy method | 6 | 1.002 |
| `integrationsection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `integrationsection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `integrationsection` | `__mul__` | overload * operator | 8 | 1.002 |
| `integrationsection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `integrationsection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `integrationsection` | `__repr__` | disp method | 22 | 1.002 |
| `integrationsection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `integrationsection` | `__str__` | string representation | 3 | 1.002 |
| `integrationsection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `integrationsection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `integrationsection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `integrationsection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `integrationsection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `integrationsection` | `printheader` | print header | 7 | 1.002 |
| `integrationsection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `integrationsection` | `write` | Write the script to a file. | 39 | 1.002 |
| `interactionsection` | `__add__` | overload addition operator | 24 | 1.002 |
| `interactionsection` | `__and__` | overload and operator | 7 | 1.002 |
| `interactionsection` | `__copy__` | copy method | 6 | 1.002 |
| `interactionsection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `interactionsection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `interactionsection` | `__mul__` | overload * operator | 8 | 1.002 |
| `interactionsection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `interactionsection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `interactionsection` | `__repr__` | disp method | 22 | 1.002 |
| `interactionsection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `interactionsection` | `__str__` | string representation | 3 | 1.002 |
| `interactionsection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `interactionsection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `interactionsection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `interactionsection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `interactionsection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `interactionsection` | `printheader` | print header | 7 | 1.002 |
| `interactionsection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `interactionsection` | `write` | Write the script to a file. | 39 | 1.002 |
| `pipescript` | `__add__` | overload + as pipe with copy | 12 | 1.002 |
| `pipescript` | `__copy__` | copy method | 6 | 1.002 |
| `pipescript` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `pipescript` | `__getitem__` | return the ith or slice element(s) of the pipe | 24 | 1.002 |
| `pipescript` | `__iadd__` | overload += as pipe without copy | 6 | 1.002 |
| `pipescript` | `__init__` | constructor | 26 | 1.002 |
| `pipescript` | `__len__` | len() method | 3 | 1.002 |
| `pipescript` | `__mul__` | overload * as multiple pipes with copy | 9 | 1.002 |
| `pipescript` | `__or__` | Overload | pipe operator in pipescript | 51 | 1.002 |
| `pipescript` | `__repr__` | display method | 29 | 1.002 |
| `pipescript` | `__setitem__` | modify the ith element of the pipe p[4] = [] removes the 4th element p[4:7] = [] removes the elements from position 4 to 6 p[2:4] = p[0:2] copy the elements 0 and 1 in positions 2 and 3 p[[3,4]]=p[0] | 58 | 1.002 |
| `pipescript` | `__str__` | string representation | 3 | 1.002 |
| `pipescript` | `clear` |  | 16 | 1.002 |
| `pipescript` | `do` | Execute the pipeline or a part of the pipeline and generate the LAMMPS script. | 118 | 1.002 |
| `pipescript` | `do_legacy` | Execute the pipeline or a part of the pipeline and generate the LAMMPS script. | 99 | 1.002 |
| `pipescript` | `dscript` | Convert the current pipescript object to a dscript object. | 115 | 1.002 |
| `pipescript` | `generate_report` | Generates a comprehensive report for specified variables and writes it to a file. | 124 | 1.002 |
| `pipescript` | `getUSER` | getUSER get USER variable getUSER(idx,varname) | 9 | 1.002 |
| `pipescript` | `header` | Generate a formatted header for the pipescript file. | 33 | 1.002 |
| `pipescript` | `join` | join a combination scripts and pipescripts within a pipescript p = pipescript.join([s1,s2,p3,p4,p5...]) | 19 | 1.002 |
| `pipescript` | `list_multiple_values` | Lists all occurrences and values of multiple variables across the pipeline scripts. | 17 | 1.002 |
| `pipescript` | `list_values` | Lists all occurrences and values of a specified variable or all variables across the pipeline scripts. | 118 | 1.002 |
| `pipescript` | `plot_multiple_value_distributions` | Plots the distribution of elements for multiple variables across specified scopes. | 54 | 1.002 |
| `pipescript` | `rename` | rename scripts in the pipe p.rename(idx=[0,2,3],name=["A","B","C"]) | 17 | 1.002 |
| `pipescript` | `script` | script the pipeline or parts of the pipeline s = p.script() s = p.script([0,2]) | 50 | 1.002 |
| `pipescript` | `setUSER` | setUSER sets USER variables setUSER(idx,varname,varvalue) | 9 | 1.002 |
| `pipescript` | `write` | Write the combined script to a file. | 27 | 1.002 |
| `runsection` | `__add__` | overload addition operator | 24 | 1.002 |
| `runsection` | `__and__` | overload and operator | 7 | 1.002 |
| `runsection` | `__copy__` | copy method | 6 | 1.002 |
| `runsection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `runsection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `runsection` | `__mul__` | overload * operator | 8 | 1.002 |
| `runsection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `runsection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `runsection` | `__repr__` | disp method | 22 | 1.002 |
| `runsection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `runsection` | `__str__` | string representation | 3 | 1.002 |
| `runsection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `runsection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `runsection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `runsection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `runsection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `runsection` | `printheader` | print header | 7 | 1.002 |
| `runsection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `runsection` | `write` | Write the script to a file. | 39 | 1.002 |
| `script` | `__add__` | overload addition operator | 24 | 1.002 |
| `script` | `__and__` | overload and operator | 7 | 1.002 |
| `script` | `__copy__` | copy method | 6 | 1.002 |
| `script` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `script` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `script` | `__mul__` | overload * operator | 8 | 1.002 |
| `script` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `script` | `__pow__` | overload ** operator | 8 | 1.002 |
| `script` | `__repr__` | disp method | 22 | 1.002 |
| `script` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `script` | `__str__` | string representation | 3 | 1.002 |
| `script` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `script` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `script` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `script` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `script` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `script` | `printheader` | print header | 7 | 1.002 |
| `script` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `script` | `write` | Write the script to a file. | 39 | 1.002 |
| `scriptobject` | `__add__` | add a structure set sortdefintions=True to sort definitions (to maintain executability) | 12 | 1.002 |
| `scriptobject` | `__eq__` | Return self==value. | 3 | 1.002 |
| `scriptobject` | `__ge__` | Return self>=value. | 2 | 1.002 |
| `scriptobject` | `__gt__` | Return self>value. | 2 | 1.002 |
| `scriptobject` | `__init__` | constructor | 30 | 1.002 |
| `scriptobject` | `__le__` | Return self<=value. | 2 | 1.002 |
| `scriptobject` | `__lt__` | Return self<value. | 2 | 1.002 |
| `scriptobject` | `__ne__` | Return self!=value. | 2 | 1.002 |
| `scriptobject` | `__or__` | overload | or for pipe | 6 | 1.002 |
| `scriptobject` | `__str__` | string representation | 3 | 1.002 |
| `scriptobjectgroup` | `__add__` | overload + | 32 | 1.002 |
| `scriptobjectgroup` | `__init__` | SOG constructor | 18 | 1.002 |
| `scriptobjectgroup` | `__or__` | overload | or for pipe | 6 | 1.002 |
| `scriptobjectgroup` | `__str__` | string representation | 3 | 1.002 |
| `scriptobjectgroup` | `group_generator` | Generate and return a group object. | 28 | 1.002 |
| `scriptobjectgroup` | `<lambda>` |  | 1 | 1.002 |
| `scriptobjectgroup` | `mass` | Generates LAMMPS mass commands for each unique beadtype in the collection. | 89 | 1.002 |
| `scriptobjectgroup` | `<lambda>` |  | 1 | 1.002 |
| `scriptobjectgroup` | `select` | select bead from a keep beadlist | 11 | 1.002 |
| `scriptobjectgroup` | `struct` | create a group with name | 10 | 1.002 |
| `statussection` | `__add__` | overload addition operator | 24 | 1.002 |
| `statussection` | `__and__` | overload and operator | 7 | 1.002 |
| `statussection` | `__copy__` | copy method | 6 | 1.002 |
| `statussection` | `__deepcopy__` | deep copy method | 8 | 1.002 |
| `statussection` | `__init__` | constructor adding instance definitions stored in USER | 14 | 1.002 |
| `statussection` | `__mul__` | overload * operator | 8 | 1.002 |
| `statussection` | `__or__` | overload | or for pipe | 19 | 1.002 |
| `statussection` | `__pow__` | overload ** operator | 8 | 1.002 |
| `statussection` | `__repr__` | disp method | 22 | 1.002 |
| `statussection` | `__rshift__` | overload right  shift operator (keep only the last template) | 10 | 1.002 |
| `statussection` | `__str__` | string representation | 3 | 1.002 |
| `statussection` | `_iadd__` | overload addition operator | 8 | 1.002 |
| `statussection` | `detect_variables` | Detects variables in the content of the template using an extended pattern to include indexed variables (e.g., ${var[i]}) if `with_index` is True. | 42 | 1.002 |
| `statussection` | `do` | Generate the LAMMPS script based on the current configuration. | 77 | 1.002 |
| `statussection` | `getallattributes` | advanced method to get all attributes including class ones | 4 | 1.002 |
| `statussection` | `header` | Generate a formatted header for the script file. | 37 | 1.002 |
| `statussection` | `printheader` | print header | 7 | 1.002 |
| `statussection` | `tmpwrite` | Write the script to a temporary file and create optional persistent copies. | 92 | 1.002 |
| `statussection` | `write` | Write the script to a file. | 39 | 1.002 |
