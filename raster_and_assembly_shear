...
# %% Raster files names
output_destination = "shear_apparatus" # e.g. "visco_h_hertzbc"
output_name = "shear_salted_resolution_%s" %(format(res)) # e.g. "h_%s" % (format(h))
...
groups = group(groupID = ['fixed_bottom'], y=['EDGE',0.0002]) & \
         group(groupID = ['moving_top'], y=[(Sensor_h+Substance_h+Top_h)*Substance_h*mm-0.0002,'EDGE'])
...
moves = translation(group=["fixed_bottom"],fixIDv = "$ fixplate") & \
        translation(vx=v_plate,group=["moving_top"],fixIDv = "$ translatetopplate") & \
        run(runs=2000000)
