...
%
%       Vgrid = interp3SPH(XYZ,y,XYZgrid [,W,V])
%
%   Inputs:
...
%
%
%   See also: buildVerletList, interp3SPH, interp2SPH, kernelSPH, packSPH
...
if ~iscell(GridVerletList) || length(GridVerletList)~=kg
    error('the supplied VerletList (%d atoms) does not match the number of grid points',length(GridVerletList),g)
end
kv = length(V);
...
for i=1:kg
    ...
    if mod(i,200)==0 || (etime(t_,t1_)>0.5) %#ok<*DETIM> 
        ...
        screen = dispb(screen,'[GridPoint %d:%d] INTERP3SPHCerlet | elapsed %0.1f s | done %0.1f %% | remaining %0.1f s', ...
